...
async def echo(event):
    ...
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "`Reply to a User's message to echo his messages`")
    legendevent = await edit_or_reply(event, "`See Magic`")
...
async def echo(event):
    ...
    if sis_echo(chat_id, user_id):
        try:
            ...
            await edit_or_reply(event, "Ai Chat has been stopped for the user")
    else:
        await edit_or_reply(event, "The user is not activated with Ai Chat")
...
async def chatbot(event):
    ...
    if input_str:
        ...
        try:
            ...
            await edit_or_reply(event, "Deleted chatbot for all enabled users in all chats.")
    else:
        ...
        try:
            ...
            await edit_or_reply(event, "Deleted chatbot for all enabled users in this chat")
...
async def echo(event):  # sourcery no-metrics
    ...
    if input_str:
        ...
        group_chats = ""
        if len(lsts) <= 0:
            return await edit_or_reply(event, "There are no chatbot enabled users")
        for chatbot in lsts:
        ...
        if len(lsts) <= 0:
            return await edit_or_reply(event, "There are no chatbot enabled users in this chat")
