...
async def _(event):  # sourcery no-metrics
    ...
    try:
        ...
        umsg = "X" if chat_per.send_messages else "✓"
        umedia = "X" if chat_per.send_media else "✓"
        usticker = "X" if chat_per.send_stickers else "✓"
        ugif = "X" if chat_per.send_gifs else "✓"
        ugamee = "X" if chat_per.send_games else "✓"
        uainline = "X" if chat_per.send_inline else "✓"
        uembed_link = "X" if chat_per.embed_links else "✓"
        ugpoll = "X" if chat_per.send_polls else "✓"
        uadduser = "X" if chat_per.invite_users else "✓"
        ucpin = "X" if chat_per.pin_messages else "✓"
        uchangeinfo = "X" if chat_per.change_info else "✓"
        res += "\nصلاحيات المجموعة : \n"
        res += f"« `الرسائل`: `{umsg}`\n"
