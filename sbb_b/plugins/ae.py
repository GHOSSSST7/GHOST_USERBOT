...
from ..helpers.utils import reply_id, _jmthonutils, parse_pre, yaml_format, install_pip, get_user_from_event, _format
...
from ..helpers.utils import _jmthonutils, _format, reply_id
CACHE = {}
...
async def on_plug_in_callback_query_handler(event):
    ...
    sqllist.rm_from_list("pmoptions", event.query.user_id)
    await event.edit(text)
@sbb_b.ar(admin_cmd(pattern="الحماية (تشغيل|ايقاف)(?: |$)(.*)"))
...
async def pmpermit_on(event):
    ...
    if input_str == "ايقاف":
        ...
        await edit_delete(event, "**🝳 ⦙  امـر الـحـمـايـة بـالـفـعـل مُـمَـكـن لـحـسـابـك  🌿**")
@sbb_b.ar(admin_cmd(pattern="(ق|قبول)(?:\s|$)([\s\S]*)"))
...
@sbb_b.ar(admin_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)"))
...
@sbb_b.ar(admin_cmd(pattern="مرفوض(?:\s|$)([\s\S]*)"))
...
@sbb_b.ar(admin_cmd(pattern="مقبول(?:\s|$)([\s\S]*)"))
...
@sbb_b.ar(admin_cmd(pattern="المقبولين(?: |$)(.*)"))
