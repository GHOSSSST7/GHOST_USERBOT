...
class CatVC:
    ...
    async def join_vc(self, chat, join_as=None):
        if self.CHAT_ID:
            return f"انت موجود فى مكالمة فى مجموعة {self.CHAT_NAME}"
        if join_as:
            try:
        ...
        try:
            await self.app.join_group_call(
                chat_id=chat.id,
                stream=AudioPiped("resources/Silence01s.mp3"),
            ...
            try:
                ...
                return "يجب ان تكون ادمن لبدء مكالمة ."
    ...
    async def pause(self):
        if not self.PLAYING:
            return "لا يوجد شئ شغال لايقافة ."
        if not self.PAUSED:
            await self.app.pause_stream(self.CHAT_ID)
            self.PAUSED = True
        return f"تم الايقاف مؤقتا فى {self.CHAT_NAME}"

    async def resume(self):
        if not self.PLAYING:
            return "لا يوجد شئ شغال لاستئنافة ."
        if self.PAUSED:
            await self.app.resume_stream(self.CHAT_ID)
            self.PAUSED = False
        return f"تم الاستئناف فى {self.CHAT_NAME}"
